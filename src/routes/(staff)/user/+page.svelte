<script lang="ts">
	...
	let loading = true;
	let users: Array<{ name: string; email: string; emailVerified: string }> = [];

	// Simulate data fetching
	setTimeout(() => {
		users = [
			{
				name: 'Muhammad Arya Firmanyah',
				email: 'aryafirmansyah200404@gmail.com',
				emailVerified: '20/09/2024'
			}
		];
		loading = false;
...
<div class="border-2 border-gray-200 bg-white rounded-lg grid grid-cols-1 divide-y">
	<div class="p-4 flex justify-between">
		<h2 class="block text-base font-semibold text-gray-900">User list</h2>
	</div>
	<div class="p-4">
		<div class="relative overflow-x-auto sm:rounded-lg">
			...
			{#if loading}
				<div class="flex justify-center items-center py-10">
					...
					<span class="ml-2 text-gray-700">Loading users...</span>
				</div>
			{:else}
