<script lang="ts">
	...
	let monthSalary: Date | null = null;
	...
	onMount(() => {
		setTimeout(() => {
			...
			employeeName = { value: 'Arya Firmansyah', label: 'Arya Firmansyah' };
			monthSalary = new Date();
			salaryFields['positionAllowance'] = 1000000;
			salaryFields['bpjsAllowance'] = 1000000;
			salaryFields['taxAllowance'] = 1000000;
			salaryFields['positionDeduction'] = 1000000;
			salaryFields['bpjsDeduction'] = 1000000;
			salaryFields['taxDeduction'] = 1000000;
			anotherSalaryAllowance = [
				{ description: 'Allowance 1', amount: 100000, showDetails: true },
				{ description: 'Allowance 2', amount: 200000, showDetails: true },
				{ description: 'Allowance 3', amount: 300000, showDetails: true }
			];
			anotherSalaryDeduction = [
				{ description: 'Deduction 1', amount: 100000, showDetails: true },
				{ description: 'Deduction 2', amount: 200000, showDetails: true },
				{ description: 'Deduction 3', amount: 300000, showDetails: true }
			];
	...
	function formatDate(date: Date) {
		...
		let month = (date.getMonth() + 1).toString().padStart(2, '0'); // tambahkan 1 karena bulan dimulai dari 0
		return `${year}-${month}`;
	}
...
<div class="flex flex-col gap-2">
	<Breadcrumbs />
	<h1 class="text-3xl text-gray-900 font-semibold">Edit Salary</h1>
</div>
			...
			<div class="p-4">
				<div class="mb-6">
					...
					<input
						id="monthPicker"
						type="month"
						value={monthSalary ? formatDate(monthSalary) : ''}
									...
									<div>
										...
										<div class="flex gap-5 mb-6 text-sm font-medium text-gray-500">
											<button on:click={() => handleCollapseAll('allowance', false)}
												>Collapse all</button
											>
											<button on:click={() => handleCollapseAll('allowance', true)}
												>Expand all</button
											>
										</div>
										<div class="grid gap-6 mb-6 md:grid-cols-1">
									...
									<div>
										...
										<div class="flex gap-5 mb-6 text-sm font-medium text-gray-500">
											<button on:click={() => handleCollapseAll('deduction', false)}
												>Collapse all</button
											>
											<button on:click={() => handleCollapseAll('deduction', true)}
												>Expand all</button
											>
										</div>
										<div class="grid gap-6 mb-6 md:grid-cols-1">
