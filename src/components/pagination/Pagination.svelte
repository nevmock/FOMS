<!-- <script lang="ts">
	...
	console.log(page, limit, recordsTotal);
	...
	const handlePrev = (): void => {
		if (page > 1) {
			page -= limit; // Update the current page
		}
		location.assign(`?start=${page}`);
	};

	// Handle next button click
	const handleNext = (): void => {
		if (page + limit < recordsTotal + limit) {
			page += limit; // Update the current page
		}
		location.assign(`?start=${page}`);
	};
	...
	const getPageNumbers = (): any[] => {
		...
		if (totalPages <= 5) {
			...
			for (let updatedPage = currentPage; updatedPage < updatedPage + 2; updatedPage++) {
		...
		<ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
			...
			{#each getPageNumbers() as pageNumber}
				<li>
					<button
						on:click={() =>
							(page = (pageNumber - 1) * limit + 1) && location.assign(`?start=${page}`)}
						class="flex items-center justify-center px-3 h-8 leading-tight {(pageNumber - 1) *
							limit +
							1 ===
						page
			...
			<li>
				<button
					...
					aria-label="Next"
					disabled={page + limit === totalPages * limit + 1}
				>
					Next
				</button>
			</li>
